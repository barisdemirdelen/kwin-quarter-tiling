...
function QuarterLayout(geometry, max) {
  ...
  this.tile = function(count) {
    for (var i = 0; i < this.tiles.length; i++) {
      this.tiles[i].x = (i === 0 || i === 3) ? this.geometry.x + this.gaps : this.geometry.x + this.pane.x +
          this.gaps / 2;
      this.tiles[i].y = (i === 0 || i === 1) ? this.geometry.y + this.gaps : this.geometry.y + this.pane.y +
          this.gaps / 2;
      this.tiles[i].width = (i === 0 || i === 3) ? this.pane.x - this.gaps * 1.5 : this.geometry.width -
          this.pane.x - this.gaps * 1.5;
      this.tiles[i].height = (i === 0 || i === 1) ? this.pane.y - this.gaps * 1.5 : this.geometry.height -
  ...
  this.move = function(client, index) {
    ...
    this.pane.x = this.clip(this.pane.x, 80, this.geometry.width - 80);
    this.pane.y = this.clip(this.pane.y, 80, this.geometry.height - 80);
  };
}
...
function MasterLayout(geometry, max) {
  ...
  this.tile = function(count) {
    for (var i = 0; i < this.tiles.length; i++) {
      ...
      this.tiles[i].y = (i === 0 || i === 1) ? this.geometry.y + this.gaps : (i === 2) ? this.geometry.y +
          this.pane.yt + this.gaps / 1.5 : this.pane.yb + this.gaps / 3;

      this.tiles[i].width = (i === 0) ? this.pane.x - this.gaps * 1.5 : this.geometry.width - this.pane.x -
          this.gaps * 1.5;

      this.tiles[i].height = (i === 0) ? this.geometry.height - this.gaps * 2 : (i === 1) ? this.pane.yt -
          this.gaps * 1.333 : (i === 2) ? this.pane.yb - this.pane.yt - this.gaps * 1.333 : this.geometry.height -
          this.pane.yb - this.gaps * 1.33;
    }
  ...
  this.move = function(client, index) {
    this.pane.x += (index === 0) ? client.geometry.width - this.tiles[index].width : this.tiles[index].width -
